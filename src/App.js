...
import { addClass } from "./redux/slice/classSlice";
import { addTeacher } from "./redux/slice/teacherSlice";
import { addParent } from "./redux/slice/parentSlice";
import { addSection } from "./redux/slice/sectionSlice";
...
const App = () => {
  ...
  const getClass = async () => {
    const getClass = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/admin/getAllClass`
    );
    dispatch(addClass(getClass.data));
  };
  const getStudent = async () => {
    const getStudent = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/admin/getAllStudents`
    );
    dispatch(addStudent(getStudent.data));
  };
  const getTeacher = async () => {
    const getTeacher = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/admin/getAllTeachers`
    );
    dispatch(addTeacher(getTeacher.data));
  };
  const getParent = async () => {
    const getParent = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/admin/getAllParent`
    );
    dispatch(addParent(getParent.data));
  };
  const getSection = async () => {
    const getSection = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/admin/getAllSection`
    );
    dispatch(addSection(getSection.data));
  };
