                      ...
                      ? classInfo?.map((item, i) => {
                          return (
                            <tr key={i}>
                              ...
                              <td className="fw-semibold">{item.section?.map((section,ind)=>{return( <div key={ind} onClick={()=>navigate(`/teacher/student/${section._id}`)} className="btn btn-success mx-1">{section.section}</div> )})}</td>
